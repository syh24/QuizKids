...
- 프로젝트 규모가 크지 않고, 복잡한 business logic이나 query logic이 없어서 Service layer, Repository layer는 생략하였다.
  ...
  * 근거: 프로젝트를 진행할 때 주어진 시간안에 개발하는 것이 가장 중요하다고 생각했다. 따라서 시간이 부족하다고 판단하여 개발 시간을 단축할 수 있는 방향으로 설계를 진행했다. 또한 확장성에 대해서는 고객의 새로운 요구사항이나 확장을 해야하는 상황이 왔을 때 해도 늦지 않다고 생각했고,
       미리 확장성을 고려하여 설계를 하면 비용(시간)이 많이들뿐더러 실제로 확장으로 이어지지 않을 시 손해를 볼 수도 있다.
