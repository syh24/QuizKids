	...
	return (
		<div className="Spottable">
			...
			<div className="ml-32 Spottable">
				...
				<Button
					color={sex === 'M' ? 'yellow' : null}
					onClick={handleUserSex}
					className="Spottable"
					size="small"
				>
					남성
			...
			<div className="mt-8 ml-32 Spottable">
				...
				{ageTypes.map((ageType, index) => (
					<Button
						className="Spottable"
						color={index === age ? 'yellow' : null}
						size="small"
						key={index}
						onClick={handleUserAge}
					>
						{ageType}
					</Button>
				))}
			...
			{showProfileSelection ? (
				<ProfileSelection onClose={toggleProfileSelection} setIdx={setImgIdx} />
			) : null}
		</div>
	);
};
